...
class RedisSessionStore(SessionStore):
    def __init__(self, redis, salt, *args, **kwargs):
        ...
        self.normal_session_store =  werkzeug.contrib.sessions.FilesystemSessionStore(path, session_class=OpenERPSession)
    ...
    def save(self, session):
        ...
        try:
            return self.redis.set(key, data, ex=ONE_WEEK_IN_SECONDS)
        except Exception as ex:
            return self.normal_session_store.save(session)
    ...
    def delete(self, session):
        try:
            ...
            self.normal_session_store.delete(session)
    ...
    def get(self, sid):
        ...
        try:
            if self.redis:
                ...
                self.redis.expire(key, ONE_WEEK_IN_SECONDS)
                data = cPickle.loads(saved) if saved else {}
        except Exception as ex:
            data = self.normal_session_store.get(sid)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                openerp.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method),
...
def session_gc(session_store):
    if random.random() < 0.001 and not hasattr(session_store, 'redis'):
        # we keep session one week
        last_week = time.time() - 60 * 60 * 2
...
class Root(object):
    ...
    def __init__(self):
        self._loaded = False
        self.redis_salt = None #'-RMsSz~]3}4[Bu3_aEFx.5[57O^vH?`{X4R)Y3<Grvq6E:L?6#aoA@|/^^ky@%TI'
        redis_params = get_redis_parmas()
        #self.redis_instance = redis.Redis(host='localhost', port=6379, db=0, password='') or False
        try:
            self.redis_instance = redis.Redis(host=redis_params.get('redis_host', 'localhost'),
                                         port=int(redis_params.get('redis_port', '6379')),
                                         db=int(redis_params.get('redis_session_db', '0')),
                                         password=redis_params.get("redis_password", ''))
        except Exception as ex:
            self.redis_instance = False
